...
def detect(opt):
    ...
    half = opt.half and opt.mlu  # half precision only supported on CUDA
    ...
    if opt.mlu:
        ...
        if opt.jit:
            ...
            example = torch.randn(1, 3, opt.img_size, opt.img_size).float()
            trace_input = torch.randn(1,3,640,640,dtype=torch.float)
            if opt.half: #1
                example = example.type(torch.HalfTensor)
                trace_input = trace_input.type(torch.HalfTensor)#1
...
#        model.half()  # to FP16
    ...
    for path, img, im0s, vid_cap in dataset:
        timer = Timer()
        if opt.mlu and opt.half:#1
            img = torch.from_numpy(img).float()
#            print("img",img)
#            img = Variable(img.type(torch.HalfTensor)) if opt.half and opt.mlu else Variable(img.type(torch.FloatTensor))#1
            img=img.half()
        else:#1
            img = torch.from_numpy(img).float()
...
#        img /= 255.0  # 0 - 255 to 0.0 - 1.0
        ...
        if opt.mlu:
            ...
            total_hardware += timer1.elapsed()
            pred = pred.cpu().type(torch.FloatTensor) if opt.half else pred.cpu()
            print("pred",pred)
            box = get_boxes(pred)
            print("box",box[0])
...
if __name__ == '__main__':
    ...
    if opt.compute_map:
        ...
        print("coco_eval.params.imgIds",coco_eval.params.imgIds)
        coco_eval.evaluate()
